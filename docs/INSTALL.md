...
`v0.1.20` and the archive name with the release and build you want; the
...
```powershell
Get-FileHash -Algorithm SHA256 .\veilvoice-v0.1.20-windows-x86_64.zip
...
| `--version v0.1.20` | a specific release rather than the latest |
...
|---|---|
| `-Yes` | no prompts, and **no** optional components |
| `-Version v0.1.20` | a specific release |
...
veilvoice verify file veilvoice-v0.1.20-linux-x86_64.tar.gz     --sums SHA256SUMS --sig SHA256SUMS.asc
```
...
veilvoice verify file veilvoice-v0.1.20-linux-x86_64.tar.gz --sha256 <their hash>
