...
run seven times, on every push to `main` that touched `wiki/`, and **all seven
failed**, each after about six seconds, each at the same step and for the same
reason:

    Cloning into 'wiki-repo'...
    remote: Repository not found.
    fatal: repository 'https://github.com/tilas01/veilvoice.wiki.git/' not found

The workflow's own error message was right about the cause, which is the
unusual part of this one. A GitHub wiki is a git repository at
`<repo>.wiki.git`, and enabling the wiki in Settings does not create it; it is
created when a first page is saved. `has_wiki` is true on this repository and
has been, so the Wiki tab exists, the setting looks done, and the repository
behind it does not exist. The workflow said so in eleven lines of prose, to a
log nobody opened for four days.

**So the whole wiki has never been published.** Everything written up under
roadmap item 178 about the wiki being the manual, everything `tools/docs/wiki.py`
generates, and the two hundred and two pages behind it: none of it has ever
been on the wiki. The website's copy is what readers have had, and the website
is generated by a different tool in a different workflow, which is why nothing
looked broken from outside.

**Two things are wrong here and only one of them is the wiki repository.**

The first is the missing repository, and it needs a person: this is a
one-time action in the web interface, and it is now listed with the other
maintainer-only steps. The workflow tries to create the repository itself
before asking, by initialising one locally and pushing it, because a workflow
that can fix itself should, and the attempt costs a second and loses nothing
if GitHub refuses it.

The second is the one worth the finding number. **`wiki.yml` ran on `main`
only.** So a wiki change pushed to `dev`, which is every wiki change, since
`dev` is where development happens, got no result from this workflow at all,
and `dev`'s commit list showed no cross beside it. A branch with no result
reads exactly like a branch that passed. The failure was therefore invisible
twice over: on `dev` because nothing ran, and on `main` because the only
pushes to `main` are release merges, which is the one moment nobody is looking
for a documentation workflow to be red.

That is the fourth time this shape is written up here. F-176 and F-177 were
checks that existed and were in no workflow; F-197 was CI itself wired to the
branch development had left; this is a workflow wired to the branch nobody
pushes to. The property they share is that **the absence of a result is not a
result**, and no amount of reading a commit list will show it.

`wiki.yml` is two jobs now. `check` runs on both branches and on pull
requests, and asserts what `publish` needs before `publish` is the thing that
discovers it: the pages match their sources, every `[[link]]` resolves, and
every file in `wiki/` is something a wiki can hold. `publish` still runs from
`main` alone, which was always right and is not what was broken: what is
published describes what has been released.

The third of those checks is new, and it is the one this would have caught
going the other way. `unpublishable()` refuses a subdirectory, a file that is
not Markdown, and a page name outside what a flat wiki can hold. The publish
step is `cp wiki/*.md wiki-repo/`, so a page written into a subdirectory by a
generator deriving a name from a source path would have been silently left
behind, on `main`, after a release merge.
...
and nothing comparing the two. F-71 was two hand-typed copies of a number,
F-101 a download page linking files that were never published, F-103 a
screenshot compared against a file written by the same command.

Every command in `README.md`, `docs/USER_GUIDE.md` and `docs/INSTALL.md` is now
checked against clap's own tree: the subcommand has to exist, and a long flag
has to belong to the subcommand it is shown beside. Against the definition the
program is built from, never a list kept next to it, which would be one more
copy to drift.

**The first version of this test was blind to half of what it claimed.** It
walked every occurrence of the word and skipped anything it could not resolve,
so a made-up flag failed the build and a made-up subcommand did not:
`veilvoice frobnicate` was added to the guide and the test passed. A test whose
name is broader than its reach is worse than no test, because it answers the
question nobody then asks again.

What it was missing is a way to tell a command from a sentence. "veilvoice"
appears in these documents as prose, as sample output and as something to type,
and only the last is checkable. An invocation is now taken to be what a reader
could copy: the whole of an inline code span, or a line inside a fenced block,
beginning with the program's name. That admits `veilvoice anonymise ...` and
excludes `● veilvoice is now using your microphone`, which sits in a fence and
is output.

All three behaviours were checked by trying them: an invented subcommand fails,
an invented flag fails, and the alert line does not.

The limit is worth stating, because the test's name invites more than it does.
It settles whether a documented command *exists*. Whether it *works* is not
checkable from the argument tree, and is what running it is for: F-110 itself
would have passed this test, because `turn` is a real keyword and the plan file
