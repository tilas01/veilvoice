...
/// The help screen for `veilvoice verify`, written out rather than derived.
///
/// Every other help screen in this program is generated by `clap` from the type
/// that defines the subcommand. This one is not, because it is the screen
/// somebody reads when they are trying to decide whether to trust a download
/// they have already got, which is a piece of writing rather than a list of
/// options. It opens with the one line to read if they read nothing else.
///
/// The two tables under it *are* generated, by [`Loudness::table`] and
/// [`Status::table`], so the meanings of the words this screen uses cannot drift
/// from the words the program prints.
const USAGE: &str = "\
...
/// The longer explanation, printed when a reader asks for it.
///
/// Its subject is the one thing about verification that is genuinely easy to get
/// wrong: that a signature proves who published a file and a reproducible build
/// proves what is in it, and that neither claim implies the other. Kept separate
/// from [`USAGE`] because somebody checking a download needs one screen and
/// somebody deciding what the check is worth needs the other.
const EXPLAIN: &str = "\
