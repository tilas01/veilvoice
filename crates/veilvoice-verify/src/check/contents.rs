    ...
    archive
        ...
        .map(|member| {
            ...
            let verdict = match std::fs::symlink_metadata(&path) {
                ...
                Ok(_) => match crate::check::sha256_file(&path) {
                    Err(why) => Verdict::Unreadable(why.to_string()),
                    Ok(found) if crate::check::digests_match(&found, &member.digest) => {
                        Verdict::Matches
                    }
...
";
    ...
    fn a_file_that_is_there_and_wrong_is_not_a_pass() {
        ...
        let real = crate::check::sha256_bytes(b"the real one");
        let published = crate::check::sha256_bytes(b"the published one");
        let text = format!(
