...
//!
//! The fingerprint in it is [`crate::check::FINGERPRINT`], and the one thing
...
/// `crate::check`'s reproduce script, in the same release, says they do not
...
impl Flavour {
    ...
    /// `crate::check`'s, and a reader on one machine may follow either. They
    /// answer to one enumeration so that they cannot describe different
    /// machines.
    fn system(self) -> crate::check::reproduce::System {
        match self {
            Flavour::Linux => crate::check::reproduce::System::Linux,
            Flavour::MacOs => crate::check::reproduce::System::MacOs,
            Flavour::Bsd => crate::check::reproduce::System::Bsd,
        }
    }

    /// The command that checks a file against `SHA256SUMS`.
    ///
    /// Asked of [`crate::check::reproduce::System`] rather than answered
...
pub fn shell(flavour: Flavour) -> String {
    let fingerprint = crate::check::FINGERPRINT;
...
mod tests {
    ...
    fn the_script_carries_the_fingerprint_the_programs_use() {
        for flavour in Flavour::ALL.iter().copied() {
            assert!(
                shell(flavour).contains(crate::check::FINGERPRINT),
    ...
    /// which verifies a signature and a hash list, and `crate::check`'s,
