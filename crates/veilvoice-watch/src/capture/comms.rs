...
use crate::capture::programs::Purpose;
...
///
/// The same reading of the process list [`crate::capture::programs`] uses, and with the
...
pub fn running() -> (Vec<&'static Comm>, Vec<String>) {
    let (names, problems) = crate::capture::processes::running();
    let found = COMMS
        .iter()
        .filter(|comm| {
            comm.processes
