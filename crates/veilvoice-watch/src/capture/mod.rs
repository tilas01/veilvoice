...
pub mod comms;
// The process listing moved to `proc` when a second feature needed
// it. Re-exported under the old name so nothing else in this crate had to
// change, and so a reader following `crate::capture::processes` still lands somewhere.
pub(crate) mod processes {
    pub(crate) use crate::proc::running;
}
pub mod programs;
