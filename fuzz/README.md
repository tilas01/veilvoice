...
| `release_contents` | the release contents list | its paths decide which files a verifier opens |
...
```bash
for t in container_header lock_file wav_chunks wav_preflight guard_manifest \
         hybrid_keys release_contents; do
...
The other five targets have no seeds and do not need any: `wav_chunks`,
`wav_preflight`, `hybrid_keys`, `guard_manifest` and `release_contents` manage
between twelve million and three hundred million inputs in ten minutes, and
find their own structure in seconds. `guard_manifest`'s minimised corpus alone
is 3.9 MB, which is a lot of committed bytes for a target that rediscovers text
in no time at all, and `release_contents` reads a text format of the same
shape.
