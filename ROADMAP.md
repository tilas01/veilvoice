...
| 180 | **Release notes for a reader, with the technical half a click away**: a release opens with the files, how to check them, and a plain list of what is new and what was fixed. Everything else, the reproducibility table per platform, the audit findings, the dependency changes and the measurements, moves behind a link to a document of its own. Both are generated from `CHANGELOG.md` as they are now, so this is where the same text is shown rather than a second copy of it | **planned** | 4 |
...
**178 has landed early**, on `dev` and therefore in v0.1.23: it is prose and a
pair of guards over the documentation generators, so it was not waiting on any
of the window work ahead of it and holding it back would only have left the
manual incomplete for a release longer. **177**, **180** and **183** are what
