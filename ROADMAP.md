...
| 164 | **Verifying any copy of VeilVoice, from anything, in one step**: the verifier answers a precise question and asks the reader to assemble the inputs first. Point it at an archive instead, and it takes the archive apart itself: the signature, the hash list, the contents manifest and the signing key that came with it, then every file inside, reporting each. The key an archive carries is **never** trusted for the answer: its fingerprint is compared against the one compiled into this binary, and a key that does not match is the whole verdict, because a download that brings its own key and its own signature verifies perfectly against itself and means nothing. And a whole-machine pass: find every copy of VeilVoice on this computer, installed or unpacked, old or current, remember where they were found, and verify each against the published sums for the version it claims to be, saying plainly for any version it cannot reach a signed list for. Both from `veilvoice verify` and from the Verify tab, with everything the verifier does today still there. **The archive half is in**: `veilvoice verify archive` takes a `.tar.gz` or a `.zip` apart and hashes every file inside it against the signed contents list without unpacking anything, `auto` does the same for each archive it finds, and the key a download carries is compared by fingerprint against the compiled-in one and never used to verify. The whole-machine pass is what remains | **next** | 15 |
