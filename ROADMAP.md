...
| 139 | **A video of who said what, from the Studio**: the *preview page* draws a circle per speaker in their own colour and lights whoever is talking; the **video file** does not, and this row used to say it did. What a render produces today is the veiled audio over a black picture, because nothing here draws video frames: `ffmpeg::command` builds the frame-sequence command and the command line only prints it, for a `frames/` directory a person would have to fill themselves. So this is two things rather than one addition: draw the frames, and put a level beside each speaker's name that moves while they talk, so the picture shows the shape of the conversation rather than just its cast. Names in a monospace face, which is the one that stays legible scaled down and does not reflow when a name is long. Single-person recordings get the same treatment as a group of eight, veiled or unveiled, because a solo take is a group of one and should not need a different path | **planned** | 8 |
