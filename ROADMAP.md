...
2. **167**, and it is the largest of the group for a reason: every button that
   ...
   and the workers it creates are what 169, 176 and 179 all hang off.
...
4. **168**, the alignment pass, with the guard that keeps it. Roadmap item 160 did this
   for two buttons and the method is now known.
5. **170** and **176**, which are the same decision made twice: what should
   already be true on a first run.
6. **171**, the tour as an overlay, which needs 169 and 170 to exist before it
...
**173**, **174** and **175**: the decoy passphrase and convincing decoys, the
stealth toggles, and the Browser's export with its thumbnails, its quality
settings and its lossless audio. Each writes to disk or changes what is on
screen at a level the audit has to read carefully, which is why they are one
release and not spread across three.
...
**178** and **177**: the wiki as the whole manual, and the real window running
on the website, compiled to WebAssembly from the same commit. The wiki first,
because it is the thing a reader needs and the demonstration is the thing that
makes them want it. **180** goes with them, since a release page rewritten for
a reader belongs beside a manual written for one.
...
manual incomplete for a release longer. **177** and **180** are what remains of
this group.
