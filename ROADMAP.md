...
| 138 | **v0.1.19, after the audit that covers everything**: the Studio, the Browser, the decoys, the setup and the BSD reproducibility all in one release, and the full audit of marker 127 run over the whole repository before the tag exists. Not the code alone: the documentation, the website, the packaging, the scripts and the reproducibility, and the optimisation pass, because a release audited in part is a release described inaccurately | **planned** | 12 |
