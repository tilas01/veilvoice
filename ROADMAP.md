...
| 147 | **Several microphones at once, veiled and metered each**: one input per guest, each veiled by its own engine with its own seed and destination voice, mixed into the one output a call or a recorder hears. **The capture path is built**: `veilvoice_audio::room`, with a recorder per guest and one for the mix, one sample rate agreed across every device before anything opens, a ring per guest so one microphone's clock drifting is that guest's dropped samples rather than everybody's, and the honest account the row asked for as a number rather than a promise: the load is the sum of every engine's realtime factor against the deadline they share, because the output callback runs all of them before it returns. The mix is summed and clipped and **never limited**, since a limiter is a dynamics processor and this program's whole claim is about what changes a voice. **What is left is the tab that drives it**: a guest list in the Studio with a microphone and a name each, the bars per guest, and takes stored per guest beside the mix | **next** | 8 |
