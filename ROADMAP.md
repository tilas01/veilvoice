...
| 147 | **Several microphones at once, veiled and metered each**: a capture path that opens one input per guest, veils each with their own engine and seed, and mixes the result, so an interview with everybody in the room on their own microphone is veiled live rather than after the fact. This is the half of marker 133 that could not be built from what was here: `LiveSession` opens **one** input, and everything downstream of it assumes one. It is the work, not the drawing: the bars 133 built are per speaker already and would show these the moment there were several to show. Wants an honest account of what it costs, because one engine per guest is one FFT chain per guest on the same deadline | **planned** | 20 |
