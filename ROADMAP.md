...
| 133 | **Every guest, veiled and plain, side by side**: two bars per speaker, what went in and what came out, so the difference is visible rather than asserted. The row used to say "live" and to place this in group mode, and neither is right today: group mode is a panel for a recording that already exists, it never opens a device, and the live path opens **one** input, so there is no per-guest live signal anywhere in this tree to draw. That makes this two separate things and the row now says which. The buildable half is **during a render**: the plan already says who speaks when, so each speaker's input and output level can be drawn as the render walks the file, which answers the question about a recording. The other half, several guests on several microphones at once, is a multi-input capture path that does not exist and is the actual work. The one-microphone live case already has its two bars, in the Studio | **planned** | 8 |
