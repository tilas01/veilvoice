...
| 178 | **The wiki is the whole manual, and the two copies are one**: what exists is the documents converted. What is missing is the part written for somebody who has just downloaded this: installing it on their own system, checking the download, what `veilvoice guard` is for and what a changed file means, updating and what to expect when they do, and every screen and command explained in the order somebody meets them. Written in the format GitHub's wiki accepts, published to both the wiki and the website from one source, and checked in CI so the two cannot drift | **planned** | 12 |
