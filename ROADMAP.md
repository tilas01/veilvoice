...
| 139 | **A video of who said what, from the Studio**: the *preview page* draws a circle per speaker in their own colour, lights whoever is talking, and now moves a level under each name. **The level is done.** A lit circle says whose turn it is and nothing about whether they are mid-sentence or mid-pause, and those looked identical for as long as the turn lasted; the bar is drawn from the same envelope as the waveform beneath it, so the two cannot disagree, and somebody whose turn it is not shows nothing rather than a small amount. It is the mix, given to whoever the plan says is speaking, which is the same thing while one person talks and is not where two turns overlap: both show the same bar, and the guide says so rather than implying a separation the renderer does not produce. **What is left is drawing the frames.** The video file is still veiled audio over a black picture: `ffmpeg::command` builds the frame-sequence command and nothing fills the `frames/` directory. That needs two things this tree does not have, and neither is a small addition: a rasteriser, because the drawing is SVG and no ffmpeg can be assumed to read it, and a font, because names are text and text is the hard half. Pulling in a full SVG rasteriser would undo the argument the ffmpeg module already makes about large C libraries, so it is a bitmap monospace face and a few hundred lines of circle, rectangle and PNG, written here. One thing established while building the level and worth not rediscovering: the picture only changes when the envelope column changes, which is a few hundred times for a whole recording rather than 108,000, so the frames are emitted at those points and held, not one per frame at thirty a second. Single-person recordings get the same treatment as a group of eight, veiled or unveiled, because a solo take is a group of one and should not need a different path | **next** | 6 |
