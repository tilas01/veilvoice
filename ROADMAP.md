...
| 163 | **ffmpeg present before it is needed, and asked for rather than assumed**: three commands need it, `import`, `video` and the Studio's render, and each found out at the moment it was used and printed the install line. That is the right refusal and the wrong time: somebody learns their recording cannot be turned into a video after they have finished making it. **Landed so far**: `veilvoice_setup::needs` asks once, in one place, whether ffmpeg and GnuPG are here and whether this copy is installed or portable, and a no is remembered per companion in a text file the reader can open and delete, because a question asked at every launch is a question people learn to dismiss without reading. `veilvoice record` says before the recording what turning it into a video will need, rather than after it, and says nothing at all when ffmpeg is there or the reader has said no. Portable is never reported as a fault, which a test asserts. **Still to come**: the offer to fetch each one. The download is checked against a hash published for that platform before anything is run, the licence is shown for anything that is not free software, and the window gets the same notice at launch that the command line now gives. A copy of ffmpeg is not shipped inside the archives: it is eighty megabytes per platform that this project cannot build reproducibly, cannot sign, and would then be answerable for the security of. Fetching a named build and checking it is the same promise the verifier makes about VeilVoice itself, which is the only form of this that is honest | **next** | 5 |
...
| 167 | **Nothing in the window ever freezes, and nothing flashes a terminal**: several buttons do their work on the thread that draws. The window stops answering, a console window appears and vanishes on Windows, and the only sign of progress is that the application has gone white. Every one of them moves to a worker: looking for GnuPG, opening a vault, decrypting in the Browser, hashing for the verifier, the companion probes, the graphics probe, and any subprocess at all, which is spawned with its console suppressed. In their place, in the window, a smooth indicator beside the control that started it, what it is doing in words, and an estimate **only where one can honestly be given**: a hash of a known length can say how far through it is; a key derivation cannot, and will say so rather than invent a bar. A test drives each of these and fails if a frame takes longer than a frame should Every one of them runs on a worker now and reports back through a channel the drawing thread reads with `try_recv`, never waits on. Two had to be moved twice, because the first move stopped at the button and the work the button led to stayed behind: the file picker was on a worker and the render it chose a folder for was not, and the unlock was on a worker while the folder it unlocked was read on the frame the answer arrived. Every subprocess in both crates goes through one wrapper that suppresses the console, which is read out of the source by a test rather than watched for, because a console window that appeared for a tenth of a second is not a thing any test can see. The indicator is one function and it makes the estimate rule structural: a bar only exists where something was given a total, which is the verifier's hash against the file's length and a round of decoys against the number asked for, and everywhere else it is a spinner with the reason in a sentence, which is the key derivation, the ffmpeg render and the companion look. A test refuses a bar drawn anywhere but there. Two tests hold the rest: one reads every drawing function in the crate and names the line that waits, and one drives five panels headlessly and fails on a frame over 25 milliseconds | **done** | - |
...
| 169 | **One loading animation, used everywhere, that respects reduced motion**: the window has a spinner in some places, a changed label in others and nothing at all in most. One indicator, drawn once, with the sentence beside it saying what is happening, used by every worker roadmap item 167 creates. Under the system's reduced-motion setting it becomes a static progress statement rather than a spinning thing, because that setting is somebody saying movement hurts There is one now, `progress::strip`, and the twelve spinners are gone. It takes the resolved motion setting rather than reading the preference itself, since that is resolved once a frame against the system setting and an environment override and a second reader of it is a second answer. The indeterminate case is a bar and not a spinner because a bar can hold still: egui's spinner reads no setting and takes none, so every panel that drew one was a panel ignoring somebody who had said movement hurts. The travelling highlight and the still third of a bar were already written, in Setup, with the reasoning in their own comment; that drawing moved rather than being written a second time. Two tests hold it: one reads the whole crate and refuses a spinner or an `egui::ProgressBar` anywhere, and one drives the indicator at two times a second apart and fails if it moved under reduced motion, or if it did not move when motion was allowed | **done** | - |
...
| 171 | **The tour is an overlay, and it sets the application up**: it was a card per tab that replaced the body of the window and was shown once. It is a walkthrough now, drawn over the window as a modal so the tab a stop describes is on screen while it describes it, and it can be asked for again from Settings under Interface at any time. The setup comes before the descriptions, because a passphrase is a decision and a tab is a paragraph: the app lock, encryption at rest, the decoy passphrase, whether this copy is installed or portable, and a walk through every heading in Settings, read from the menu's own list rather than written out again. It offers rather than acts where acting is a commitment, so a password is set here and installing this copy puts somebody in front of the Install tab instead. It adapts to what is already set: a stop with nothing left to offer is dropped before it starts rather than shown greyed out, and the app lock is not asked for again thirty seconds after the first-run card asked. What is stored is the list of stops shown, which is what carries the setup to the people who most need it: somebody who has had VeilVoice installed for a year has every tab key stored and none of the setup ones, so the next launch offers them the setup, once. Escape closes it and a stray click on the shade deliberately does not, and a passphrase typed into it is wiped on every way out. The decoy stop is written and gated off until roadmap item 173 gives it something to set | **done** | - |
...
| 179 | **An updater that does the update**: the check reported a version and stopped, which left the reader to download, verify and replace by hand. `veilvoice update` fetches the archive published for this platform along with `SHA256SUMS`, `SHA256SUMS.asc` and `CONTENTS.sha256`; verifies the signature against the key compiled into the copy already running, then the archive, then the contents list, then **every file inside the archive**, hashed where it lies without unpacking anything; opens it only then, with the system's own `tar`, and hashes what came out against the same signed list, because an extractor is a program too; and replaces the binaries beside the running copy, old one moved aside first and moved back if the write fails, so at no point is there no program there. All of the checking is `veilvoice-verify`'s existing code rather than a second implementation of it, which is why roadmap item 164 had to land first. It updates **the copy you are running**, not an installation it has guessed at, because portable is an ordinary way to use VeilVoice. The objection the old module stated, that an update checker which can install its own answer can be made to install somebody else's, is answered rather than dropped: the answer it installs has to carry a signature made by a key that is already inside the binary asking, and nothing still runs unless somebody asks it to. **The integrity record is handled honestly across an update**: a new installation's files are new, so the old record reported every one of them as changed, every time, which is how people learn to dismiss the one alarm that matters. The record is re-taken as part of the update, after the signature has been checked and not before, and written unsealed unless the reader is there to seal it. With an app lock set the passphrase is **required** and asked for before anything is downloaded, because an update that went ahead without it would leave new files under a sealed record of the old ones, which at the next unlock is indistinguishable from tampering. The platform labels the download URL is built from live in the release workflow as well as in Rust, so `tools/audit/release_targets.py` fails a build on drift either way: a platform told nothing is offered for it, or a build asking for an archive that has never existed | **done** | - |
...
| 181 | **Titles look like titles**: the headings in Settings are the words `appearance`, `motion`, `interface`, `security` and `storage`, in lower case, in a menu where every other label is capitalised. They are titles and are written as ones The heading and the deep-link name are now two fields rather than one. They were the same string doing both jobs, so capitalising what a reader sees would have renamed `--settings-page appearance`, renamed the capture file and moved the address the website publishes that picture at, none of which is what was asked for. The key stays lower case and a test asserts it is the heading lowercased, so the two are checked against each other rather than left to agree by habit | **done** | - |
...
| 183 | **A picture of what changed, published with every release**: a release was described in words in two places and the only image either carried was the project's banner, which is the same picture for every version and says nothing about this one, so every link to a release posted anywhere showed the same card. A card per release now: the version and the changes that release led with, drawn by `tools/release/card.py` in the site's own palette, with the same hand-drawn pixel font and the same PNG writer the banner and the mark already use, so this adds no dependency. **The words are not a second copy of the notes**: they are read out of `CHANGELOG.md` by the same functions `tools/site/releases.py` uses to build the page, and the two entries that use bold for a label rather than for a change are named with the reason rather than caught by a rule about length, because any rule short enough to catch `**In short**` also catches a real heading. An entry with no headline changes at all falls back to the sentence the releases page already shows beside that version. **No date**, deliberately: `CHANGELOG.md` carries none and neither does the site, so there is nothing in the tree to draw and a date from the clock would differ between a build and its check. Shown at the top of each release's entry on the website, and at the top of the notes on GitHub, addressed at that release's own tag so it cannot start showing a later card. Checked as every other generated thing is, in `tools/verify.py` and in CI: a card that does not match what `CHANGELOG.md` says fails a build, and a card left behind by an entry that has gone is removed rather than kept, because a picture of the wrong release is worse than none | **done** | - |
...
| 186 | **A version is cut by the repository, not by remembering how**: `ci.yml` tested every push and `release.yml` built every tag, and between them sat a paragraph of `docs/CONTRIBUTING.md` performed by hand: check the version, check the notes, check the items are done, check the audit round happened, merge, tag, dispatch. The one clause with machinery behind it, the refusal to publish a release with no `CHANGELOG.md` section, fired in the last step of the last job, after twelve build jobs and after `main` had already moved. A `promote` workflow inverts that order: everything askable is asked first, from a fresh checkout rather than from a working tree, and `main` moves only once all of it has passed. `tools/release/readiness.py` is the six-clause half of it and runs anywhere, so a laptop can answer what a release is still waiting on. Starting it stays a person's decision, because a version is cut when a group is finished and audited rather than on a date; everything after that decision is not. The same reading found the two checks `tools/verify.py` makes that no CI job ran, both of them needing a compiled program, which is how a recorded session disagreed with its program for two commits with the whole matrix green. F-207 and F-208 | **done** | - |
...
2. **167**, and it is the largest of the group for a reason: every button that
   ...
   **Done**, over five findings, F-212 and F-216 to F-221.
3. **169**, immediately after. 167 left the indicator behind it, one function
   with the honest-estimate rule built in, so this was the sweep rather than the
   invention. **Done**: twelve panels drew a bare spinner of their own and now
   none does, the resolved motion setting reaches every panel that draws, and a
   guard refuses the next spinner rather than trusting the sweep to have been
   thorough.
...
6. **170** and **176**, which are the same decision made twice: what should
   already be true on a first run. **170 is done**; 176 waits on 167 for the
   worker that generates a key pair without stopping the window.
7. **171**, the tour as an overlay. **Done.** It went ahead of 169 rather than
   after it: the one stop that waits on anything, the app lock, says so in a
   line of its own and moves on by itself when the worker answers, so a shared
   indicator improves it rather than being what it needs to exist. Then
   **172**, the reset, which is what the tour offers to somebody who wants to
   start again.

**182 and 186 have landed already**, on `dev` and therefore in this release:
both were broken or absent machinery rather than features, nothing in the list
above was waiting on either, and neither is a thing to leave standing for the
length of a release. A wiki that has never once published is one. The other is
a release procedure nothing checks, which this release is itself the first to
be cut by.
...
**178, 180 and 183 have landed early**, on `dev` and therefore in v0.1.23. All
three are prose and generators rather than window work, so none was waiting on
anything ahead of it, and holding them back would have left the manual
incomplete and the release notes unreadable for a release longer. **183**
followed **180** within the day rather than within the release, for the reason
above: the layout it draws from was already the new one. **177** is what
