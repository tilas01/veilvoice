...
| 116 | **One author in the history, and every commit verified**: the commit history rewritten so the whole of it matches the attribution rule this project already states, with the assistance credit staying exactly where a reader looks for it (the README, and the footer of every page) rather than in a trailer that makes a second contributor of it, and every commit in the history signed rather than only the recent ones. Trees are unchanged by this, so reproducible builds still verify | **planned** | 2 |
...
| 121 | **A studio vault that needs both keys**: every recording the studio makes, and every preview it holds, sealed into one vault whose key exists only when the app lock *and* the at-rest passphrase have both been given. Neither alone derives it, so a stolen laptop with the app unlocked opens nothing and a known recording passphrase without the app opens nothing. The same post-quantum sealing used everywhere else, held in the same page-locked, zeroizing memory, with the same honest report of how much the operating system actually agreed to lock | **planned** | 12 |
| 122 | **Recording Studio**: the place recording happens, with the vault already open because both locks were answered on the way in. Capture, monitor, preview and re-take without a plaintext file existing at any point, and a session that locks itself back up when the app does rather than staying open behind a screensaver | **planned** | 10 |
| 123 | **Recording Browser**: everything in the vault, listed with what it is and when it was made, played back by decrypting into locked memory a block at a time rather than writing a temporary file somebody would have to remember to shred. Export is a deliberate act with its own warning, because leaving the vault is the moment the protection ends | **planned** | 10 |
| 124 | **Neither lock alone, proven rather than asserted**: tests that open the vault with each secret in turn and get nothing, and a stated account of what the pair does and does not buy. It raises the cost of a stolen machine and of a guessed passphrase; it does not defeat somebody watching the process while both are entered, and that is written beside the feature rather than left implied | **planned** | 4 |
| 125 | **A pass for size and speed, changing no behaviour**: the tree read for the things that accumulate rather than the things that break. Work repeated that could be done once, allocations in paths that run per frame or per sample, generic code instantiated more times than it needs to be, dependencies pulled in for one function, and anything the compiler is doing twice. Measured before and after, with the binary size and the timings recorded, and **not one behavioural change**: every test that passed before passes after, or the change is reverted rather than argued for | **planned** | 8 |
| 126 | **Written so the next pass finds nothing**: the practices that pass establishes turned into the standing way this project is written, so optimisation stops being a thing done afterwards. Allocation kept out of per-sample and per-frame paths by construction, a dependency justified before it is added rather than removed later, and the same reading applied to every change before it is pushed rather than to the tree once a year | **planned** | - |
| 128 | **The BSDs built twice, like everywhere else**: FreeBSD, OpenBSD and NetBSD are the three platforms whose archives say `not-verified (built once, in a VM)`, which is honest and is also the only gap in the reproducibility claim. The second build happens in the same VM, with the same path remapping and the same `SOURCE_DATE_EPOCH` the other ten platforms use, and the verdict is published in the release notes beside theirs. If a BSD turns out not to reproduce, the archive says so in those words rather than quietly dropping the line | **planned** | 5 |
| 129 | **A BSD reader can check their own copy either way**: whatever the double build concludes, somebody on a BSD can verify the binary in front of them, with the program itself (`veilvoice verify`, which needs no GnuPG and no network), with their own GnuPG, or by pasting a hash into the website's checker, which hashes locally in the browser and uploads nothing. Written up per platform in the guides and the wiki rather than left as a Linux instruction somebody has to translate | **planned** | 4 |
| 130 | **The live scramble moved into the Studio**: veiling as it runs stops being a separate tab and becomes what the Studio does. The same engine, the same ratchet, the same virtual-cable routing, with the recorder already attached, so hearing yourself veiled and keeping what you said are one act rather than two screens | **planned** | 8 |
| 131 | **Both sides of the glass, kept or discarded**: record the input, the veiled output, or both, and say which before starting rather than discovering it afterwards. An unveiled take is allowed and is the one thing here that writes a recording of the real voice, so it is asked for explicitly, warned about in the same words the plaintext path uses, and never the default | **planned** | 6 |
| 132 | **Audio that says when it was interfered with**: the samples reaching the recorder are checked against what the engine produced, so a device swapped mid-session, another program capturing the same stream, or anything sitting between the two is noticed and shown rather than recorded silently. Stated limit up front: this notices interference with VeilVoice's own path and cannot vouch for a microphone that was already lying | **planned** | 10 |
| 133 | **Every guest, veiled and plain, side by side**: in group mode each speaker gets two live bars, what went in and what came out, so the difference is visible while it happens rather than asserted afterwards. Somebody hosting an interview can see at a glance that every guest is actually being veiled, which is the question that matters and the one a single output meter cannot answer | **planned** | 8 |
| 134 | **Decoy vaults, as many as you choose**: the setup asks whether to make dummy vaults and how many, sizes them from the real one so they are indistinguishable by size, and fills them with encrypted nonsense under keys that are thrown away. Cracked, one yields bytes that will not parse as anything: a decoy is not a vault with weak contents, it is a vault whose contents never existed. The count defaults from the free space actually detected rather than from a number picked here, and the honest limit is stated: this raises the cost of a search, it does not make the real vault unfindable to somebody who watches you open it | **planned** | 10 |
| 135 | **Setup inside the app, with defaults it worked out**: first run asks the questions that have real answers, in one place, with every default derived from this machine rather than assumed: how much space there is, whether the graphics driver can accelerate the window, which devices exist. Every answer says what it costs. Nothing is silently decided for somebody who wanted to decide it | **planned** | 8 |
| 136 | **Portable first, and it tells you where it is**: settings live in the working directory so a copy on a stick stays a copy on a stick, and the About tab prints the exact paths it is using rather than leaving somebody to guess. Installing later can carry those settings over or leave them, and that is a question rather than a default, because the answer differs for a shared machine and a private one | **planned** | 6 |
| 137 | **Acceleration on unless you turned it off**: the graphics path is chosen at setup from what the driver actually reports, kept on where it works, and turned off in one place for the machines where it does not. What was chosen and why is shown on the About tab beside the driver it was chosen from | **planned** | 4 |
...
| 145 | **The Recording Studio, in full, for v0.1.20**: everything markers 121 to 137 specify, built and working together rather than as parts. Recording in an environment that never lets a plaintext sample reach the disk, sealed post-quantum into a vault that needs both the app lock and the at-rest passphrase, held in page-locked zeroizing memory. It shows, at all times and both live and on replay, whether the voice being recorded is veiled or not. It has its own failsafe, separate from the application's, so a fault in the Studio stops the Studio rather than the recording. Group mode records every guest with the same guarantees. The output is rendered inside the vault, viewed and listened to inside the vault, and leaving it is an export: a deliberate act, warned about, because that is the moment the protection ends. Something like a recording studio somebody already knows how to use, that happens to be secure, rather than a security tool somebody has to learn to record with | **planned** | 30 |
| 146 | **v0.1.20, the Studio release**: 145 with the Browser, the decoys, the setup and the BSD reproducibility, tagged after its own audit round rather than on the strength of 0.1.19's. An audit covers the tree it was run on and no other | **planned** | 8 |
