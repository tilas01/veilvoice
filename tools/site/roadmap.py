...
So this reads that file and writes two things: a picture of every roadmap item
coloured by its state, and a page that puts the picture at the top and the list
under it. Neither is typed. An item whose status changes in `ROADMAP.md`
...
One square per item, in the order the roadmap lists them, grouped by the
section headings that file already has. Colour is state and nothing else:
done, next, planned, blocked.

**Area is not progress.** Every square is the same size, and an item is not a
day: roadmap item 1 is the entire signal engine and roadmap item 68 is a page of questions.
...
**Blocked is its own colour, not a shade of unfinished.** Four items are
...
# A row in one of the roadmap's tables: number, item, status, and sometimes an
# estimate. The status is bold in the file, which is what `\*\*` is doing here.
ROW = re.compile(
    r"^\|\s*(?P<number>\d+)\s*\|\s*(?P<item>.+?)\s*\|\s*"
...
# Sections of the roadmap that are a list of items. Anything else in the file
...
def items(text):
    """Every item in the roadmap, as (section, number, item, status, estimate).
    ...
    # A line that looks like an item row and does not parse is not skipped.
    #
    ...
    # `63b`'s shape: an item whose number is not a plain integer. The second is
    # a row that opens with a plain integer and does not parse, which is what
    # roadmap items 109 and 110 were for as long as their status was written `planned`
    # rather than `**planned**`: the bold-token detector could not see them
    # because the only bold text on the line was the item's own title, so they
    # were numbered, present, and in no count of anything. A numbered row is a
    # an item by definition, so a numbered row this cannot read is the same
    ...
    for candidate in (candidates[start] for start in sorted(candidates)):
        ...
        if section_of(candidate.start()) in SKIP_SECTIONS:
            continue
        raise SystemExit(
            "a row in ROADMAP.md looks like an item and does not parse:\n"
            "    %s\n"
...
def split_item(item):
    """An item's bold title, and the rest of it.
    ...
    """
    match = re.match(r"^\*\*(?P<title>.+?)\*\*\s*[:.]?\s*(?P<detail>.*)$", item,
                     re.S)
    if not match:
        return "", item
...
def prose_for(text, sections):
    ...
    This is where the reasoning behind a group of items is written, and the
...
def plain(markup):
    """An item's text without its Markdown, for a tooltip.

    Deliberately small: bold, code and links, which is everything the roadmap's
    item column actually uses. Anything else is left as it is rather than
    half-converted.
    """
...
def graphic(colours, groups, counts):
    """Every item as a square, grouped by the roadmap's own sections."""
        ...
        for index, row in enumerate(rows):
            ...
            add('<g opacity="%s"><title>%s (%s)</title>'
                ...
                % ("1" if row["status"] == "done" else "0.85",
                   docs.esc(plain(row["item"])[:90]), row["status"],
...
def film(colours, done):
    ...
    `ROADMAP.md`, so it cannot show an item as finished that is not.
    ...
    for row in done:
        lines = wrap(plain(row["item"]), columns)
...
def item_html(title, detail):
    """An item's name in bold and its explanation after it."""
    if not title:
        return docs.inline_html(detail)
    if not detail:
        return "<strong>%s</strong>" % docs.inline_html(title)
    return '<strong>%s</strong> <span class="item-detail">%s</span>' % (
...
def item_entry(row):
    ...
    """
    title, detail = split_item(row["item"])
    return ('<li id="m%d"><a class="item-link" href="#m%d" '
            'aria-label="link to item %d">%d</a> %s</li>'
            % (row["number"], row["number"], row["number"], row["number"],
               item_html(title, detail)))


def body(colours, groups, counts, generated_at, prose):
    """The page under the picture: the same items, as readable rows."""
    total = sum(counts.values())
    ...
    for row in outstanding:
        title, detail = split_item(row["item"])
        add('<tr id="m%d"><td><a class="item-link" href="#m%d" '
            'aria-label="link to item %d">%d</a></td><td>%s</td><td>%s</td>'
            "<td>%s</td></tr>"
            % (row["number"], row["number"], row["number"], row["number"],
               item_html(title, detail), docs.esc(row["status"]),
    ...
    for title, rows in groups:
        ...
        if not finished:
            continue
        slug = anchor(title)
        add('<h3 id="%s">%s <a class="item-link" href="#%s" '
...
def build(root):
    ...
    if not rows:
        raise SystemExit(
            "no roadmap items were found in ROADMAP.md.\n"
            "  The tables are parsed by their shape: a number, an item, a\n"
