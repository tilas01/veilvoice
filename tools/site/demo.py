...
# The screens that are not tabs, and the sentence under each.
#
# A picture of the Settings tab is a picture of whichever page it opens on, so
# the other pages are photographed separately by the capture scripts and
# captioned here. The page list itself is read out of `settings.rs`, like the
# tabs: this is only the sentence, and a page that gains a picture without
# gaining one fails the build rather than appearing blank.
PAGE_NOTES = {
    "motion": "Animation, the frame rate the window draws at, and whether the "
              "mark moves. It follows the system's own reduced-motion setting "
              "unless you say otherwise, and says which way it read it.",
    "interface": "Which tabs the window offers. An installed copy never shows "
                 "the install tab, because a program offering to install "
                 "itself when it already is tells you something untrue.",
    "security": "How long the window waits before locking itself, in the "
                "list or typed as `90m`. Off unless you turn it on, and a "
                "long render does not count as using the window.",
    "storage": "Where the settings file is, what is in it, and what resetting "
               "it does. Nothing is hidden behind a menu item called "
               "advanced.",
}

# And the screens that are neither a tab nor a page of one.
OTHER_NOTES = {
    "locked": ("The window with the app lock on, which is all anybody who "
               "picks your computer up gets to see: the mark, the word "
               "locked, and somewhere to type. Not the file you were working "
               "on, not where the lock lives, and not how many attempts have "
               "been made."),
}
...
def shot(key, label, note, why):
    """One picture, checked to exist, with the sentence that goes under it."""
    name = "gui-%s.png" % key
    path = os.path.join(ROOT, "assets", "screenshots", name)
    if not os.path.exists(path):
        raise SystemExit(
            "%s and %s does not exist, so the walkthrough would show a\n"
            "  heading with nothing under it.\n"
            "  Run: tools/shots/gui.sh on Linux, or tools/shots/gui.ps1 on "
            "Windows" % (why, os.path.relpath(path, ROOT)))
    if not note:
        raise SystemExit(
            "%s and there is no sentence for it here.\n"
            "  Every picture in the walkthrough is captioned; add one rather\n"
            "  than shipping a screenshot nobody has explained." % why)
    return {
        "key": key,
        "label": label,
...
def shots():
    """Each screen, its screenshot, and the sentence that goes under it.

    The tab list comes from `app.rs`, the settings pages from `settings.rs`,
    and the pictures from `assets/screenshots`. Pairing them here is what keeps
    the walkthrough honest: a screen with no photograph, or a photograph with
    no sentence, stops the build rather than rendering a gap somebody has to
    notice.

    Three groups, because the window has three kinds of screen and one picture
    per tab reaches only the first of them. The Settings tab's menu has five
    pages behind it and a photograph of the tab is a photograph of one; and the
    locked window is not a tab at all, which is why the control somebody asks
    about most had never been in any picture on this site.
    """
    out = [shot(tab["key"], tab["label"], TAB_NOTES.get(tab["key"]),
                "the application has a %r tab" % tab["key"])
           for tab in tabs()]

    # The page the tab opens on is skipped: `gui-settings.png` is already it.
    # Which one that is comes from the order, as it does in the capture
    # scripts, rather than from a name written here as well.
    ...
    for key, note in OTHER_NOTES.items():
        out.append(shot(key, key.capitalize(), note,
                        "the walkthrough lists a %r screen" % key))
    return out
