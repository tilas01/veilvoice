...
BANNER_W = 1200
# Tall enough for a title and two lines of subtitle, which is what most of them
# need. A banner whose subtitle runs longer grows instead of cutting it: see
# `banner_svg`.
BANNER_H = 150
# One line of subtitle, in the 15px stack the subtitle is drawn in.
BANNER_LINE = 19.0
...
def banner_svg(colours, title, subtitle, kind):
    ...
    # Wrapped before anything is drawn, because how many lines the subtitle
    # needs decides how tall the banner is.
    #
    ...
    # description should not be abbreviated. Height is cheap and a truncated
    # sentence is not.
    #
    # Two lines still measure exactly 150, so every banner that already fitted
    # is unchanged, byte for byte.
    height = BANNER_H + max(0, len(lines) - 2) * BANNER_LINE
    ...
    add('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 %d %d" '
        ...
        % (BANNER_W, height, BANNER_W, height, esc(title)))
    ...
    add('<rect x="0" y="0" width="%d" height="%d" rx="10" fill="%s"/>'
        % (BANNER_W, height, colours["bg"]))
    add('<rect x="0.5" y="0.5" width="%d" height="%d" rx="10" fill="none" '
        'stroke="%s"/>' % (BANNER_W - 1, height - 1, colours["border"]))
    ...
    for index, bar in enumerate(heights):
        ...
        add('<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" rx="3" '
            'fill="%s" opacity="%.2f"/>'
            % (x, mid - half, width, half * 2, colour, 0.55 + 0.45 * bar))
        x += step
    ...
    # this.
    #
    # `lines` and the height that holds all of them were worked out at the top
    # of this function, before the canvas was sized.
