...
def diagram_svg(colours, nodes, edges, width=880):
    """Lay the same graph out as inline SVG, for readers with no JavaScript.
    ...
    if not nodes:
        return ('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 %d 40" '
                'width="100%%" role="img" aria-label="no items"><text x="10" '
                'y="25" font-family="%s" font-size="13" fill="%s">'
                'nothing to draw</text></svg>\n' % (width, MONO, colours["muted"]))
    ...
    # Widest row decides the canvas; every row is centred inside it.
    row_widths = {}
    for number, row in rows.items():
        row_widths[number] = sum(box[n["id"]][0] for n in row) + gap_x * (len(row) - 1)
    canvas_w = max(max(row_widths.values()) + 40.0, width)

    placed = {}
    y = 20.0
    for number in sorted(rows):
        row = rows[number]
        height = max(box[n["id"]][1] for n in row)
        x = (canvas_w - row_widths[number]) / 2.0
        for node in row:
            w, h = box[node["id"]]
            placed[node["id"]] = (x, y + (height - h) / 2.0, w, h)
            x += w + gap_x
        y += height + gap_y
    canvas_h = y - gap_y + 20.0
...
def html_page(colours, depth, title, description, body, fingerprint):
    ...
    add = out.append
    add('<!doctype html>')
    add('<!-- SPDX-License-Identifier: CC-BY-NC-SA-4.0 -->')
    add('<!-- GENERATED by tools/docs/generate.py. Do not edit. -->')
